...
class TestFullSession:
    ...
    def test_quitting_immediately_prints_goodbye_with_no_message(
        self, capsys
    ):
        run(['7'])
        out = capsys.readouterr().out
    ...
    def test_encrypting_before_entering_a_message_shows_an_error(
        self, capsys
    ):
        run(['3', '7'])
        out = capsys.readouterr().out
    ...
    def test_decrypting_after_entering_a_new_message_forgets_the_old_key(
        self, capsys
    ):
        ...
        run([
            '1', 'first message', '3', '50',   # encrypt with offset 50
            '1', 'second message',              # overwrite the message
            '4', '99',                          # must require typing an offset now
            '7',
        ])
        out = capsys.readouterr().out
